...
AJSC_HOME="${AJSC_HOME-/opt/app/validation-service}"

JARFILE="$AJSC_HOME/validation-service.jar"
...
if [ -z "$CONFIG_HOME" ]; then
    ...
    echo "E.g. CONFIG_HOME=${AJSC_HOME}/config"    
    exit 1
fi
...
PROPS="${PROPS} -Dlogback.configurationFile=${AJSC_HOME}/${LOGBACK_FILE}"
