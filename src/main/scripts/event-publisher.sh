...
# This script attempts to publish a message to the specified topic.
# 
# Arguments:
...
if [ "$#" -ne 6 ]; then
    ...
    echo "Usage: `basename $0` <url list> <topic> <partition> <message> <api key> <api secret>" 1>&2
    exit 1
fi
base_dir=$(dirname $0)/..
...
java -cp ".${CPSEP}$base_dir/extJars/*" com.att.ecomp.event.client.test.TestEventPublisher "$1" "$2" "$3" "$4" "$5" "$6" > /dev/null || exit 1
